...
* Users have name, username, password, email, profile pic.

Acceptance:
* Sign-up by adults for under 13. (roadmap)
...
* Teacher: Create Classrooms and upload files.
...
Acceptance:
* User can successfully edit their profile saving changes to the database.
...
Assumptions:
* User progress will be stored in a user database on the back end.
...
Assumptions:
Teachers have a school code to distinguish them from students to allow them to create classrooms.
...
* Teachers can add special notes to help their students with topics.
* Teacher can make a custom lesson plan from existing games (roadmap).
* Teachers and students can communicate via message boards.
...
* Message board is functional to ask questions/seek help if necessary.
* Lessons have links to additional information for students seeking to further their knowledge of a topic. (roadmap)
...
* Game responds to user changes in the Code Editor.
...
* Correct answers can be tested for validity (front end testing. wrap code in try-catch?.)
* Student’s entered code will reload if they revisit a lesson.
* Safeguards are in place to prevent harmful code from being entered by users. (roadmap)
...
* Admins can delete messages if content is inappropriate. (roadmap)
* Users can edit messages that they posted (roadmap).
* Users can delete messages that they posted. (roadmap)
* Users can reply to other messages to create a chain, or they can start a new chain.

###Game Ideas

####Name: Sandwich Maker (roadmap)
...
How to Play: User must move their character from a starting position to a spot marked ‘x’ using prebuilt functions.
...
####Name:  Untitled Card Game (roadmap)
